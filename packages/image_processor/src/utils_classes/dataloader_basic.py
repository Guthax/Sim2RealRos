...
from utils_classes.cvat_preprocessor import CVATPreprocessor
DEBUG = True
...
class DTSegmentationDataset(torch.utils.data.Dataset):
    ...
    CVAT_XML_FILENAME = "images/annotations_basic.xml"
    SEGM_LABELS = {
        'Background': {'id': 0, 'rgb_value': [0, 0, 0]}, # black
        'Ego Lane': {'id': 1, 'rgb_value': [128, 64, 128]}, # green
        'Middle Lane': {'id': 2, 'rgb_value': [157,234, 50]}, # yellow
    }
    
    def __init__(self):
        super(DTSegmentationDataset, self).__init__()
        # Store the list of all image names
        self.imgs = CVATPreprocessor.get_all_image_names(self.CVAT_XML_FILENAME)
    ...
    def __getitem__(self, idx):
        ...
        # load the image
        img = Image.open(os.path.join(self.PATH_TO_IMAGES,image_name)).convert("RGB")
    ...
    @staticmethod
    def label_img_to_one_hot(label_img):
        """
        Converts a label image (with one channel per label) to an RGB image.
        """
        one_hot = np.zeros((len(DTSegmentationDataset.SEGM_LABELS), label_img.shape[0], label_img.shape[1]), dtype=np.uint8)
        for i , (label, label_info) in enumerate(DTSegmentationDataset.SEGM_LABELS.items()):
            channel = (label_img == label_info['id']).astype(int)
            one_hot[i] = channel
        return one_hot
